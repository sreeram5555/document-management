...
const Signup = () => {
    ...
    setError('');
    try {
        await API.post('/auth/signup', formData);  // fixed endpoint
        setLoading(false);
    ...
    if (err.response) {
        setError(`Backend error: ${err.response.status} - ${err.response.data?.message || err.message}`);
    } else if (err.request) {
            ...
            <div className="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        ...
                        <input type="text" name="username" required onChange={handleChange} className="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    <div>
